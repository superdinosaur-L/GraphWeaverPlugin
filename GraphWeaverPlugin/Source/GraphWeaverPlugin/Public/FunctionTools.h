...
UCLASS()
class GRAPHWEAVERPLUGIN_API UFunctionTools : public UBlueprintFunctionLibrary
{
	GENERATED_BODY()
public:
	UFunctionTools();
	virtual ~UFunctionTools() override;

	static FString FlipString(FString& SourceString);

	UFUNCTION(BlueprintPure, Category = "GraphWeaver|FunctionTools")
	static FString& GetSelfNameFromDescription(UPARAM(ref)FGraphNodeDescription& SourceDes);

	UFUNCTION(BlueprintPure, Category = "GraphWeaver|FunctionTools")
	static FString& GetSelfIdFromDescription(UPARAM(ref)FGraphNodeDescription& SourceDes);
