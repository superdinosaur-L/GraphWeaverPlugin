...
public:
	...
	//在UFunctionTools::ModGraphViewBaseAttri_NotManuallyCallModGraphViewBaseAttri_NotManuallyCall中设置
...
public:
	//检查Parent下面已连接的是否有SelfName或者SelfId和自己一样的情况,并且自动处理.有则返回true,没有返回false.此方法要求在构建图
	//的过程中与父子相关的代码必须放在第一时间处理,否则可能会污染兄弟相关的信息
	//bool CheckSameNode_Parent(int32 ParentIndex, int32 SelfIndex);
	...
	//UFUNCTION(BlueprintCallable, Category = "AnyGraph|GraphView", meta = (ToolTip = "设置RealNodes"))
	//void SetRealNodes();
